...
~~~c
// Denotes that puts will perform read-only access on the memory pointed to by ptr.
...
// Denotes that fgets will perform write-only access up n character on the memory pointed to by buff and read-write access on the memory pointed to by stream.
...
Using this attribute allows the compiler to optimize the generated code without regard to what would happen if the annotated function ever did return. In addition, the compiler can generate a diagnostic for any function declared as `noreturn` that appears to return to its caller[^clang-noreturn] and `noreturn` functions can improve the accuracy of other diagnostics, e.g., by helping the compiler reduce false warnings for uninitialized variables[^gcc-noreturn].
...
The `tainted_args` attribute can be used both in function declarations, and field declarations containing function pointers. In the latter case, any function used as an initializer of such a callback field will be treated as being called with tainted arguments.
