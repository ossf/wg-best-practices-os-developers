...
Clang currently supports only the form of the `malloc` attribute without arguments, but instead provides the `ownership_returns`, `ownership_takes`, and `ownership_holds` attributes that interact with the Clang static analyzer.
