...
### Clearly state how to report vulnerabilities

Clearly state how to report vulnerabilities so vulnerability finders can easily find it (e.g., in a `SECURITY.md` or `README` file). The CRA requires vulnerability reporting, and it's always been best practice to clearly state how to report vulnerabilities. Normally this uses private reporting so attackers won't learn about vulnerabilities at the same time as the maintainers. For more information, see the [OpenSSF Guide to implementing a coordinated vulnerability disclosure process for open source projects](https://github.com/ossf/oss-vulnerability-guide/blob/main/maintainer-guide.md#readme).
