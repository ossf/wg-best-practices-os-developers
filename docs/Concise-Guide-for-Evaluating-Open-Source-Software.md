...
6. **How will you become aware of security announcements & updates?**
   1. Are there regular releases or only a mainline branch? How will you approach updating this software (every version, minor only, etc?)
   2. Is there a newsletter or some other announcement board where security alerts and mitigations are announced?
   3. If you are a significant user/contributor of this of dependency you may want to consider requesting pre-release (embargoed) notice of future updates if available.

8. **Are there instructions on how to report vulnerabilities?** See the [Guide to implementing a coordinated vulnerability disclosure process for open source projects](https://github.com/ossf/oss-vulnerability-guide/blob/main/maintainer-guide.md#guide-to-implementing-a-coordinated-vulnerability-disclosure-process-for-open-source-projects) for guidance to OSS projects.
...
10. **What is the software’s license?** Licenses are technically not security, but licenses can have a significant impact on security and sustainability. Ensure every component has a license, that it’s a widely-used [OSI license](https://opensource.org/licenses) if it’s OSS, and that it’s consistent with your intended use. Projects that won’t provide clear license information are less likely to follow other good practices that lead to secure software.
11. **What are the results of code evaluation?** Even a brief review of software (by you, someone you hire, or someone else), along with recent changes to it, can give you some insight. Here are things to consider:
...
As you integrate a particular dependency into your project, consider how you can plan for the future changes to the lifecycle of the depdendency. See OpenSSF's [Evaluating Usage Considerations](https://github.com/ossf/wg-best-practices-os-developers/blob/main/docs/Evaluating-Usage-Concerns.md)
