...
9. **What happens on a tentative addition?** Try adding the dependency as a test, preferably in an isolated environment (in case it's malicious).
   1. Does it exhibit malicious behaviour, e.g., does it attempt to exfiltrate sensitive data?
   2. What new indirect dependencies does it add? Is they unexpectedly many or unnecessary? Ask components to reduce runtime dependencies (e.g., by having them differentiate between runtime, development time, and test time dependencies).
